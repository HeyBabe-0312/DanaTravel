...
const ChangePass = ({ changePW, removeChangePW, user }) => {
  ...
  const checkChangePW = async () => {
    ...
    if (oldpw === "" || newpw === "" || renpw === "") {
      toast.info(t("changePass.fillAllFields"));
    } else if (oldpw.length < 8 || newpw.length < 8) {
      toast.info(t("changePass.passwordTooShort"));
    } else if (oldpw === newpw) {
      toast.error(t("changePass.passwordSame"));
    } else if (newpw !== renpw) {
      toast.error(t("changePass.passwordNotMatch"));
    } else {
      const data = {
      ...
      try {
        ...
        if (val.status === 200) {
          toast.success(t("changePass.updateSuccess"));
